...
const Projects = ({ profileId, projects: initialProjects, onDataUpdate }) => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const response = await axiosInstance.get('api/profile/projects/view', {
        headers: {
  ...
  const handleAddProject = async (e) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.response?.data?.message || 'Error processing project',
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
  ...
  const handleDeleteProject = async (projectId) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.response?.data?.message || 'Error deleting project',
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
 ...
 const getStatusColor = (status) => {
    const statusColors = {
      ...
      'Completed': 'completed','bg-blue-100 text-blue-800': 'bg-blue-100 text-blue-800',
      'Ongoing':'ongoing', 'bg-green-300 text-green-800': 'bg-green-300 text-green-800',
      ...
      <div className="flex flex-wrap items-center gap-4 mt-2">
        ...
        {project.institute && (
          <div className="flex items-center gap-2 text-gray-600">
            ...
            <span> Institue:{project.institute}</span>
          </div>
        )}
        ...
        {project.project_amt && (
          <div className="flex items-center gap-2 text-gray-600">
            ...
            <span>Amount:{project.project_amt}</span>
          </div>
        )}
        ...
        {project.co_investigators && (
          <div className="flex items-center gap-2 text-gray-600">
            <Users className="w-4 h-4 text-gray-500" />
            <span>Co-Investigatorss: {project.co_investigators}</span>
          </div>
        )}
      </div>
    </div>
